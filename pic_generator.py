...
def func_np(func):
    """Преобразует функцию для работы с numpy массивами."""
...
def scipy_method_track(method_func, f, x0, max_iter=100, eps=1e-6):
    # Поскольку библиотечные методы не возвращают траекторию, создаем обертку
    x = x0.copy()
    trajectory = [x.copy()]

    # Определяем функцию обратного вызова для отслеживания итераций
    def callback(xk):
...
def plot_2d(func, trajectories, names, title, filename, x_range=(-5, 5), y_range=(-5, 5)):
    ...
    # Построение траекторий оптимизации
...
# Функция для запуска методов и создания визуализаций
def run_visualization(func, func_sympy, func_name, start_point, x_range=(-5, 5), y_range=(-5, 5)):
    ...
    # Запуск методов и получение траекторий
    ...
    # Собираем все траектории и имена методов
    trajectories = [
        newton_golden_traj,
        newton_wolfe_traj,
        bfgs_traj,
